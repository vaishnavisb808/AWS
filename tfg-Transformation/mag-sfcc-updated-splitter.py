...
producer = KafkaProducer(bootstrap_servers='b-1.ensdevmsk.clmhx4.c2.kafka.eu-west-2.amazonaws.com:9092,b-2.ensdevmsk.clmhx4.c2.kafka.eu-west-2.amazonaws.com:9092,b-3.ensdevmsk.clmhx4.c2.kafka.eu-west-2.amazonaws.com:9092')
...
configMaster = {
    "DEMANDWAREmasterCatalog" : {
        "delimiter" : "</product>", #Mandatory parameter for xml if splitting
        "splitThreshold" : 50, #in MB, Optional Parameter default 256
        "noOfChunks" : 9, # Optional Parameter default 9
        "destination_file_type" : "xml", #Mandatory Field
    },
    "albertron" :{
        "delimiter" : "\n", #Optional Parameter for csv
        "splitThreshold" : 50, #in MB
        "destination_file_type" : "csv", #Mandatory Field
    },
    "AMGtoSFCCLocationFeed" :{
        "destination_file_type" : "xml", #Mandatory Field
    }
    }
...
def lambda_handler(event, context):
    ...
    if file_size < config.get('splitThreshold', 256) * 1024 * 1024:  # Check if file size is less than config splitThreshold in MB
        send_single_event(bucket_name, file_name, file_size, file_type, feed_name, config.get('destination_file_type'))
    else:
        try:
            split_file(bucket_name, file_name, file_size, file_type, config, feed_name, config.get('destination_file_type'))
...
def send_single_event(bucket_name, file_name, file_size, file_type, feed_name, destination_file_type):
    u = str(uuid.uuid4())
    send_event(u, bucket_name, file_name, 0, file_size - 1, 0, True, file_type, feed_name, destination_file_type)
    send_aggregate_event(u, [0], file_type, file_name, bucket_name, destination_file_type)

def split_file(bucket_name, file_name, file_size, file_type, config, feed_name, destination_file_type):
    ...
    u = str(uuid.uuid4())
    send_events_to_msk(u, bucket_name, file_name, adjusted_splits, file_type, feed_name, destination_file_type)
...
def send_events_to_msk(uuid, bucket_name, file_name, adjusted_splits, file_type, feed_name, destination_file_type):
    n = len(adjusted_splits)
    for i in range(n):
        start = adjusted_splits[i-1]+1 if i > 0 else 0
        end = adjusted_splits[i]
        send_event(uuid, bucket_name, file_name, start, end, i, i == (n - 1), file_type, feed_name, destination_file_type)

    indexes = list(range(n))
    send_aggregate_event(uuid, indexes, file_type, file_name, bucket_name, destination_file_type)
    producer.flush()

def send_event(uuid, bucket_name, file_name, start, end, index, last, file_type, feed_name, destination_file_type):
    payload = {
        "uuid": uuid,
        "event": "transform",
...
def send_aggregate_event(uuid, indexes, file_type, file_name, bucket_name, destination_file_type):
    print(destination_file_type)
    payload = {
        'uuid': uuid,
        'event': 'aggregate',
        'indexes': indexes,
    ...
    item = {
        ...
        'destination_file_type' : {'S' : destination_file_type}
    }
